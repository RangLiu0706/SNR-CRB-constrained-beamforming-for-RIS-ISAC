% This Matlab script can be used to generate Fig. 4(b) in the paper:
...
P_W_range = 10.^(P_dbm_range/10-3);
rho1_range =  [1 2 4 6 8 10]/4; %%% initial rho
...
% %%% achieved CRB
% CRB_my = zeros(1,length(P_W_range));
% CRB_max_CG = zeros(1,length(P_W_range));
% CRB_sep = zeros(1,length(P_W_range));
...
for sim = 1:1:N_sim
    ...
    for P_index = 1:1:length(P_W_range)
        ...
        [W_my,phi_my,sr,crb] = get_W_phi_CRB_SI(Prms,Channel,phi_CG0,[W_CG0 zeros(M,M)]*sqrt(P),rho1_range(P_index));
        SR_my(P_index) = SR_my(P_index) + sr(end);
        % CRB_my(P_index) = CRB_my(P_index) + crb;

        [W_CG,sr_CG,crb_CG] = get_W_with_phi_CRB_SI(Prms,Channel,phi_CG0,[W_CG0 zeros(M,M)]*sqrt(P),rho1_range(P_index));
        SR_max_CG_RIS(P_index) = SR_max_CG_RIS(P_index) + sr_CG(end);
        % CRB_max_CG(P_index) = CRB_max_CG(P_index) + crb_CG;
        ...
        % CRB_sep(P_index) = CRB_sep(P_index) + crb_sep;
...
hold on
plot(P_dbm_range,SR_max_CG_RIS,'-x','color',[0,0,0],'LineWidth',1.5)
plot(P_dbm_range,SR_sep,'-s','color',[0,0,0.8],'LineWidth',1.5)
hold off
xlabel('Transmit power {\it P} (dBm)');
