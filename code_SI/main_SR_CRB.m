...
rho1_range = [0.1 0.2 0.4 0.8 0.1 0.2]/2; %%%% initial rho
...
for sim = 1:1:N_sim
    ...
    scal = norm(C1,'fro');
    W_CG0 = sqrt(P)*get_initial_W_SI(C1/scal,Hsi/scal,M,K);
    ...
    for CRB_index = 1:1:length(CRB_range)
        ...
        [W_my,phi_my,sr,crb] = get_W_phi_CRB_SI(Prms,Channel,phi_CG0,[W_CG0 zeros(M,M)]*sqrt(P),rho1_range(CRB_index));
        SR_my(CRB_index) = SR_my(CRB_index) + sr(end);
        % CRB_my(CRB_index) = CRB_my(CRB_index) + crb;

        [W_CG,sr_CG,crb_CG] = get_W_with_phi_CRB_SI(Prms,Channel,phi_CG0,[W_CG0 zeros(M,M)]*sqrt(P),rho1_range(CRB_index));
        SR_max_CG_RIS(CRB_index) = SR_max_CG_RIS(CRB_index) + sr_CG(end);
        % CRB_max_CG(CRB_index) = CRB_max_CG(CRB_index) + crb_CG;

        [W_sep,phi_sep,sr_sep,crb_sep] = get_W_phi_sep_CRB_SI(Prms,Channel,[W_CG0 zeros(M,M)]*sqrt(P));
...
hold on
plot(CRB_range,SR_max_CG_RIS,'-x','color',[0,0,0],'LineWidth',1.5)
