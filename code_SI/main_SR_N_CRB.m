...
for sim = 1:1:N_sim
    ...
    for N_index = 1:1:length(N_range)
        ...
        [W_my,phi_my,sr,crb] = get_W_phi_CRB_SI(Prms,Channel,phi_CG0,[W_CG0 zeros(M,M)]*sqrt(P),rho1_range(N_index));
        SR_my(N_index) = SR_my(N_index) + sr(end);
        % CRB_my(N_index) = CRB_my(N_index) + crb;

        [W_CG,sr_CG,crb_CG] = get_W_with_phi_CRB_SI(Prms,Channel,phi_CG0,[W_CG0 zeros(M,M)]*sqrt(P),rho1_range(N_index));
