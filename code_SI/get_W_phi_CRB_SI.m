% Optimize the CRB-constrained joint beamforming and reflection design problem with self interference.
...
function [W,phi,Vsr,crb] = get_W_phi_CRB_SI(Prms,Channel,phi,W,rho)
...
while iter <= Nmax && res >= res_th
    ...
    q1 = trace(q1k'*inv(Rn)*q1k);q3 = 0;q6 = trace(qk'*inv(Rn)*qk);
    ...
    for k = 1:1:K+M
        u1 = u1 + U1k(:,:,k)'*inv(Rn)*q1k(:,k);
        U1 = U1 + U1k(:,:,k)'*inv(Rn)*U1k(:,:,k);
        u2b = u2b + U2k(:,:,k)'*inv(Rn)*q1k(:,k);
        U2 = U2 + U1k(:,:,k)'*inv(Rn)*U2k(:,:,k);
        z2bM = z2bM + diag(hrt')*conj(G)*inv(Rn)*q1k(:,k)*W(:,k)'*G'*diag(hrt_der') + diag(hrt_der')*conj(G)*inv(Rn)*q1k(:,k)*W(:,k)'*G'*diag(hrt');
        C2 = C2 + U1k(:,:,k)'*inv(Rn)*Z2k(:,:,k);
        q3 = q3 + q1k(:,k)'*inv(Rn)*qk(:,k);
        u3 = u3 + U1k(:,:,k)'*inv(Rn)*qk(:,k);
        u3b = u3b + Uk(:,:,k)'*inv(Rn)*q1k(:,k);
        U3 = U3 + U1k(:,:,k)'*inv(Rn)*Uk(:,:,k);
        z3bM = z3bM + diag(hrt')*conj(G)*inv(Rn)*q1k(:,k)*W(:,k)'*G'*diag(hrt');
        C3 = C3 + U1k(:,:,k)'*inv(Rn)*Zk(:,:,k);
        U4 = U4 + U2k(:,:,k)'*inv(Rn)*U2k(:,:,k);
        C4b = C4b + Z2k(:,:,k)'*inv(Rn)*U2k(:,:,k);
        Z4 = Z4 + Z2k(:,:,k)'*inv(Rn)*Z2k(:,:,k);
        u5 = u5 + U2k(:,:,k)'*inv(Rn)*qk(:,k);
        U5 = U5 + U2k(:,:,k)'*inv(Rn)*Uk(:,:,k);
        z5M = z5M + diag(hrt')*conj(G)*inv(Rn)*qk(:,k)*W(:,k)'*G'*diag(hrt_der') + diag(hrt_der')*conj(G)*inv(Rn)*qk(:,k)*W(:,k)'*G'*diag(hrt');
        Z5 = Z5 + Z2k(:,:,k)'*inv(Rn)*Zk(:,:,k);
        C5b = C5b + Z2k(:,:,k)'*inv(Rn)*Uk(:,:,k);
        C5 = C5 + U2k(:,:,k)'*inv(Rn)*Zk(:,:,k);
        u6 = u6 + Uk(:,:,k)'*inv(Rn)*qk(:,k);
        U6 = U6 + Uk(:,:,k)'*inv(Rn)*Uk(:,:,k);
        z6M = z6M + diag(hrt')*conj(G)*inv(Rn)*qk(:,k)*W(:,k)'*G'*diag(hrt');
        Z6 = Z6 + Zk(:,:,k)'*inv(Rn)*Zk(:,:,k);
        C6b = C6b + Zk(:,:,k)'*inv(Rn)*Uk(:,:,k);
    end
    ...
    rho3 = rho3*0.9;
    rho4 = rho4*0.9;
    if VCRB(iter,3) < 0.2*CRB
        rho1 = rho1*1.5;
    elseif VCRB(iter,3) > 0.8*CRB
        rho1 = rho1*0.7;
        rho2 = rho2*0.7;
        rho3 = rho3*0.7;
        rho4 = rho4*0.7;
