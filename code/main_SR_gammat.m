% This Matlab script can be used to generate Fig. 6(a) in the paper:
...
Gammat_dB_range = (0:2:10); %%% radar SNR

N_sim = 10000;
%%% achieved sum-rate
SR_my = zeros(1,length(Gammat_dB_range));
SR_max_CG_RIS = zeros(1,length(Gammat_dB_range));
SR_comm_only = zeros(1,length(Gammat_dB_range));
SR_sep = zeros(1,length(Gammat_dB_range));
% Gammat_my = zeros(1,length(Gammat_dB_range));
% Gammat_max_CG = zeros(1,length(Gammat_dB_range));
% Gammat_comm_only = zeros(1,length(Gammat_dB_range));
% Gammat_sep = zeros(1,length(Gammat_dB_range));
for sim = 1:1:N_sim
    tic
    sim
    ...
    gammat = 10^(0.1*Gammat_dB_range(1)); Prms.gammat = gammat;

    [W_comm,phi_comm,sr_comm,gammat_comm] = get_W_phi_comm(Prms,Channel,phi_CG0,sqrt(P)*W_CG0);
    SR_comm_only = SR_comm_only + sr_comm(end)*ones(1,length(Gammat_dB_range));
    % Gammat_comm_only = Gammat_comm_only + gammat_comm*ones(1,length(Gammat_dB_range));

    for gammat_index = 1:1:length(Gammat_dB_range)
        gammat_index
        ...
        SR_max_CG_RIS(gammat_index) = SR_max_CG_RIS(gammat_index) + sr_CG(end);
        % Gammat_max_CG(gammat_index) = Gammat_max_CG(gammat_index) + gammat_CG;

        [W_sep,phi_sep,sr_sep,gammat_sep] = get_W_phi_sep(Prms,Channel);
        SR_sep(gammat_index) = SR_sep(gammat_index) + sr_sep(end);
        % Gammat_sep(gammat_index) = Gammat_sep(gammat_index) + gammat_sep;
    end
    toc
end
...
figure
plot(Gammat_dB_range,SR_my,'-o','color',[0.8,0,0],'LineWidth',1.5)
hold on
plot(Gammat_dB_range,SR_max_CG_RIS,'-x','color',[0,0.5,0],'LineWidth',1.5)
plot(Gammat_dB_range,SR_comm_only,'-d','color',[0,0,0],'LineWidth',1.5)
plot(Gammat_dB_range,SR_sep,'-s','color',[0,0,0.8],'LineWidth',1.5)
hold off
xlabel('Radar SNR requirement \Gamma_t');
