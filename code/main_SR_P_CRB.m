...
rho1_range = [1 2 4 6 8 10]*2; %%% initial rho
N_sim = 10000;
%%% achieved sum-rate
...
% CRB_comm_only = zeros(1,length(P_W_range));
...
for sim = 1:1:N_sim
    ...
    for P_index = 1:1:length(P_W_range)
        ...
        [W_my,phi_my,sr,crb] = get_W_phi_CRB(Prms,Channel,phi_CG0,[W_CG0 zeros(M,M)]*sqrt(P),rho1_range(P_index));
        SR_my(P_index) = SR_my(P_index) + sr(end);
        % CRB_my(P_index) = CRB_my(P_index) + crb;

        [W_CG,sr_CG,crb_CG] = get_W_with_phi_CRB(Prms,Channel,phi_CG0,[W_CG0 zeros(M,M)]*sqrt(P),rho1_range(P_index));
        ...
        % CRB_comm_only(P_index) = CRB_comm_only(P_index) + crb_comm;
