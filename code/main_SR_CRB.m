% This Matlab script can be used to generate Fig. 6(b) in the paper:
...
Prms.P = 10.^(30/10-3); P = Prms.P; %%% power budget

CRB_range = [0.005 0.01 0.015 0.02 0.025 0.03]; %%% CRB threshold
rho1_range = [1 2 4 8 10 16]; %%%% initial rho

N_sim = 10000;
%%% achieved sum-rate
SR_my = zeros(1,length(CRB_range));
SR_max_CG_RIS = zeros(1,length(CRB_range));
SR_comm_only = zeros(1,length(CRB_range));
SR_sep = zeros(1,length(CRB_range));
% CRB_my = zeros(1,length(CRB_range));
% CRB_max_CG = zeros(1,length(CRB_range));
% CRB_comm_only = zeros(1,length(CRB_range));
% CRB_sep = zeros(1,length(CRB_range));
for sim = 1:1:N_sim
    tic
    sim
    ...
    W_CG0 = sqrt(P)*get_initial_W(C1/norm(C1,'fro'),M,K);
    ...
    Wc = sqrt(P)*get_initial_W(C1/norm(C1,'fro'),M,K);

    CRB = CRB_range(1); Prms.Crb = CRB;

    [W_comm,phi_comm,sr_comm,~,crb_comm] = get_W_phi_comm(Prms,Channel,phi_CG0,W_CG0*sqrt(P));
    SR_comm_only = SR_comm_only + sr_comm(end)*ones(1,length(CRB_range));
    % CRB_comm_only = CRB_comm_only + crb_comm*ones(1,length(CRB_range));

    for CRB_index = 1:1:length(CRB_range)
        CRB_index
        CRB = CRB_range(CRB_index); Prms.Crb = CRB;

        [W_CG,sr_CG,crb_CG] = get_W_with_phi_CRB(Prms,Channel,phi_CG0,[W_CG0 zeros(M,M)]*sqrt(P),rho1_range(CRB_index));
        SR_max_CG_RIS(CRB_index) = SR_max_CG_RIS(CRB_index) + sr_CG(end);
        % CRB_max_CG(CRB_index) = CRB_max_CG(CRB_index) + crb_CG;

        [W_my,phi_my,sr,crb] = get_W_phi_CRB(Prms,Channel,phi_CG0,[W_CG0 zeros(M,M)]*sqrt(P),rho1_range(CRB_index));
        SR_my(CRB_index) = SR_my(CRB_index) + sr(end);
        % CRB_my(CRB_index) = CRB_my(CRB_index) + crb;

        [W_sep,phi_sep,sr_sep,crb_sep] = get_W_phi_sep_CRB(Prms,Channel,[W_CG0 zeros(M,M)]*sqrt(P));
        SR_sep(CRB_index) = SR_sep(CRB_index) + sr_sep(end);
        % CRB_sep(CRB_index) = CRB_sep(CRB_index) + crb_sep;
    end
    toc
end
...
figure
plot(CRB_range,SR_my,'-o','color',[0.8,0,0],'LineWidth',1.5)
hold on
plot(CRB_range,SR_max_CG_RIS,'-d','color',[0,0.5,0],'LineWidth',1.5)
plot(CRB_range,SR_comm_only,'-x','color',[0,0,0],'LineWidth',1.5)
plot(CRB_range,SR_sep,'-s','color',[0,0,0.8],'LineWidth',1.5)
hold off
xlabel('CRB threshold');
